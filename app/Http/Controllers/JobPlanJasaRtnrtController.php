...
use App\Models\JobPlanJasaRtnrt;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class JobPlanJasaRtnrtController extends Controller
{
    /**
    ...
    {
        $job_plan_jasa = JobPlanJasaRtnrt::orderBy('id', 'desc')->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            if($request->hasFile('kak_file')){
                $validatedData['kak_file'] = FileHelper::uploadWithVersion($request->file('kak_file'), 'readiness_rtnrt/jasa/job_plan/kak/');
            }
            if($request->hasFile('boq_file')){
                $validatedData['boq_file'] = FileHelper::uploadWithVersion($request->file('boq_file'), 'readiness_rtnrt/jasa/job_plan/boq/');
            }
            $job_plan_jasa = JobPlanJasaRtnrt::create($validatedData);
    ...
    {
        $job_plan_jasa = JobPlanJasaRtnrt::find($id);
    ...
    {
        $job_plan_jasa = JobPlanJasaRtnrt::with(['readiness_jasa_rtnrt'])->where('readiness_jasa_rtnrt_id', $id)->orderby('id', 'desc')->get();
        ...
        try {
            if($request->hasFile('kak_file')){
                ...
                if ($job_plan_jasa->kak_file) {
                    FileHelper::deleteFile($job_plan_jasa->kak_file, 'readiness_rtnrt/jasa/job_plan/kak/');
                }
            }
            if($request->hasFile('boq_file')){
                $validatedData['boq_file'] = FileHelper::uploadWithVersion($request->file('boq_file'), 'readiness_rtnrt/jasa/job_plan/boq/');
                ...
                if ($job_plan_jasa->boq_file) {
                    FileHelper::deleteFile($job_plan_jasa->boq_file, 'readiness_rtnrt/jasa/job_plan/boq/');
                }
            }
